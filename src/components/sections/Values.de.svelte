...
>
	<div class="mx-auto max-w-5xl text-center">
		<h2 class="mb-6 text-4xl font-bold tracking-tight">{de.values.title}</h2>
		<p class="text-base-content/80 mx-auto max-w-3xl text-lg leading-relaxed">{de.values.lead}</p>
	</div>

	<div class="mt-20 grid w-full max-w-6xl gap-10 md:grid-cols-3">
		{#each de.values.items as item}
			<div
				class="bg-base-200 rounded-xl p-6 text-left shadow-md transition-all duration-300 hover:shadow-lg"
			>
				<svelte:component this={icons[item.icon]} class="text-primary mb-4 h-8 w-8" />
	...
	<p class="text-base-content/60 mt-20 max-w-2xl text-center text-sm italic">{de.values.closing}</p>
</section>
